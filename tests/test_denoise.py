...
from ocrd_wrap.cli import SkimageDenoise, SkimageBinarize
...
def analyse_result(ws, level, grp):
    ...
    out_images = out_pcgts.etree.xpath('//page:%s/page:AlternativeImage[contains(@comments,"despeckled")]' % level, namespaces=NAMESPACES)
    assert len(out_images) > 0, "found no denoised AlternativeImages in output PAGE file"

def test_param_json(workspace_sbb):
    run_processor(SkimageBinarize,
                  input_file_grp="OCR-D-IMG",
                  output_file_grp="OCR-D-BIN-SKIMAGE",
                  **workspace_sbb,
    )
    run_processor(SkimageDenoise,
                  input_file_grp="OCR-D-BIN-SKIMAGE",
                  output_file_grp="OCR-D-DEN-SKIMAGE",
                  parameter=json.load(open(PARAM_JSON)),
                  **workspace_sbb,
    )
    ws = workspace_sbb['workspace']
    ws.save_mets()
    analyse_result(ws, 'Page', 'OCR-D-DEN-SKIMAGE')
...
def test_regions(workspace_aufklaerung):
    ...
    ws.save_mets()
    analyse_result(ws, 'TextRegion', 'OCR-D-DEN-SKIMAGE')
...
def test_lines(workspace_aufklaerung):
    ...
    ws = workspace_aufklaerung['workspace']
    ws.save_mets()
    analyse_result(ws, 'TextLine', 'OCR-D-DEN-SKIMAGE')
