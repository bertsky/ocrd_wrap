...
from ocrd_wrap.cli import SkimageBinarize
...
def test_param_json(workspace_sbb):
    ...
    ws.save_mets()
    analyse_result(ws, 'Page', 'OCR-D-BIN-SKIMAGE')
...
def test_regions(workspace_aufklaerung):
    ...
    ws.save_mets()
    analyse_result(ws, 'TextRegion', 'OCR-D-BIN-SKIMAGE')
...
def test_lines(workspace_aufklaerung):
    ...
    ws = workspace_aufklaerung['workspace']
    ws.save_mets()
    analyse_result(ws, 'TextLine', 'OCR-D-BIN-SKIMAGE')
